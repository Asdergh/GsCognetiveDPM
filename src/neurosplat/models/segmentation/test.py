...
class DptSegmenter(nn.Module):
    
    def __init__(
        self,
        resolution: Tuple[int, int],
        checkpoints: Optional[str]="Intel/dpt-large-ade",
    ) -> None:
        
        super().__init__()
        self.model = DPTForSemanticSegmentation.from_pretrained(checkpoints)
        self.cfg = self.model.config
        self.preprocessor = DPTImageProcessor.from_pretrained(checkpoints)
    ...
    def __call__(self, img: Union[str, torch.Tensor], get_hiden_states: bool=False):
        
        if isinstance(img, str):
            img = self.transforms(img)
            print(img.size())
        
        dpt_out = self.model.dpt(img)
        hidden_states = dpt_out.hidden_states
        last_hiden_states = dpt_out.lsat_hiden_states
        intermediate_activations = dpt_out.intermediate_activations
        states = [hidden_states, last_hiden_states, intermediate_activations]
        for state in states:
            print(type(state))

        hidden_states = [
            features
            for (idx, features) in enumerate(hidden_states[1:])
            if idx in self.cfg.backbone_out_indices
        ]
...
if __name__ == "__main__":
    ...
    model = DptSegmenter(
        resolution=(224, 224),
        checkpoints="Intel/dpt-large-ade"
    )
    image = "/home/ram/Downloads/360_v2/kitchen/images_4/DSCF0658.JPG"
    result = model(image)
